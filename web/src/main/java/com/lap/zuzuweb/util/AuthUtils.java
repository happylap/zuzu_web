...
public class AuthUtils {
	...
	private static final String GOOGLE_CLIENT_ID = "846012605406-9tnrh80j8kcbcma29omhlsekot2mo0gm.apps.googleusercontent.com";
	private static final String FACEBOOK_APP_ID = "1039275546115316";
	...
	public static boolean isGoogleTokenValid(String idTokenString) throws Exception {
		...
		if (idToken != null) {
			 ...
			 * System.out.println("User ID: " + userId); String audience =
			 * (String) payload.getAudience();
			 * 
			 * String email = payload.getEmail(); boolean emailVerified =
			 * Boolean.valueOf(payload.getEmailVerified()); String name =
			 * (String) payload.get("name"); String pictureUrl = (String)
			 * payload.get("picture"); String locale = (String)
			 * payload.get("locale"); String familyName = (String)
	...
	public static boolean isGoogleTokenValid(String token, String sub) throws Exception {
		...
		try {
			 ...
			 * "iss": "https://accounts.google.com", "sub":
			 * "110169484474386276334", "azp":
			 * "1008719970978-hb24n2dstb40o45d4feuo2ukqmcc6381.apps.googleusercontent.com",
			 * "aud":
			 * "1008719970978-hb24n2dstb40o45d4feuo2ukqmcc6381.apps.googleusercontent.com",
			 * "iat": "1433978353", "exp": "1433981953", // These seven fields
			 * are only included when the user has granted the "profile" and //
			 * "email" OAuth scopes to the application. "email":
			 * "testuser@gmail.com", "email_verified": "true", "name" :
			 * "Test User", "picture":
			...
			if (!actualObj.isNull()) {
				...
				if (!StringUtils.equals(sub, _sub)) {
					...
					throw new RuntimeException("Valid Google Token Error: The token has expired. ["
							+ CommonUtils.getUTCStringFromDate(_exp_time) + "]");
				} else {
					return true;
				}
			}
	...
	public static boolean isFacebookTokenValid(String token, String userId) throws Exception {
		...
		try {
			 ...
			 * Here's an example response:
			 * 
			 * { "data": { "app_id": "1039275546115316", "application":
			 * "\u8c6c\u8c6c\u5feb\u79df", "expires_at": 1461486439, "is_valid":
			 * true, "issued_at": 1456302439, "metadata": { "sso":
			 * "iphone-safari" }, "scopes": [ "user_friends", "email",
			 * "public_profile" ], "user_id": "919778441391523" } }
			 */
				...
				if (jsonNode_data != null) {
					...
					if (!StringUtils.equals(userId, _userId)) {
						throw new RuntimeException(
								"Valid FB Token Error: " + "The 'user_id' is not matched. [" + userId + "]");
